...
class Radosbench2(Benchmark):

    def __init__(self, archive_dir, cluster, config):
        super(Radosbench2, self).__init__(archive_dir, cluster, config)
    ...
    def create_data_analyzer(self, run, host, proc):
        return Radosbench2Analyzer(self.out_dir, run, host, proc)
    ...
    def initialize(self):
        super(Radosbench2, self).initialize()
    ...
    def __str__(self):
        return "%s\n%s\n%s" % (self.run_dir, self.out_dir, super(Radosbench2, self).__str__())


class Radosbench2Analyzer(DataAnalyzer):
